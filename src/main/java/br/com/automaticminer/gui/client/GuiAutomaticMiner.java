...
public class GuiAutomaticMiner extends GuiContainer {
    ...
    protected void actionPerformed(GuiButton button) {
        // A máquina se move, mas esta GUI continua aberta com a TileEntity original.
        // Atualizamos o texto localmente e o servidor aplica a mesma ação na máquina atual.
        if (button.id == 0) te.running = !te.running;
        else if (button.id == 1) te.down = !te.down;
        else if (button.id == 2) te.rails = !te.rails;
        else if (button.id == 3) te.target = Math.max(1, te.target - 64);
        else if (button.id == 4) te.target += 64;
