            ...
            public IMessage onMessage(final MinerButtonMessage message, final MessageContext ctx) {
                ctx.getServerHandler().player.getServerWorld().addScheduledTask(new Runnable() {
                    ...
                    public void run() {
                        ...
                        TileAutomaticMiner miner = null;

                        // A máquina anda. Depois de andar a GUI ainda possui a posição antiga,
                        // então localizamos a máquina do próprio jogador pela identidade salva nela.
                        if (tile instanceof TileAutomaticMiner && ((TileAutomaticMiner) tile).isOwner(playerId)) {
                            miner = (TileAutomaticMiner) tile;
                        } else {
                            double bestDistance = Double.MAX_VALUE;
                            for (TileEntity candidate : world.loadedTileEntityList) {
                                if (candidate instanceof TileAutomaticMiner) {
                                    TileAutomaticMiner possible = (TileAutomaticMiner) candidate;
                                    if (possible.isOwner(playerId)) {
                                        double d = candidate.getPos().distanceSq(ctx.getServerHandler().player.getPosition());
                                        if (d < bestDistance) {
                                            bestDistance = d;
                                            miner = possible;
                                        }
